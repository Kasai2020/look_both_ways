...
if "SALICON_DATA_DIR" not in os.environ:
    ...
    os.environ["SALICON_DATA_DIR"] = "/media/isaac/Extreme SSD/Downloads/SALICON"
    print(os.environ["SALICON_DATA_DIR"])
...
#TODO: LOTS OF CHANGES HERE DOWN
class SALICONDataset(Dataset, utils.KwConfigClass):
    ...
    def get_data(self, img_nr):
        ...
        sal = np.reshape(sal, (480,640,1))
        sal = self.preprocess(sal, data='sal')
        fix = self.get_fixation_map(img_nr)

        if np.shape(fix) != (480,640):
            print(np.shape(fix))
        fix = np.reshape(fix, (480,640,1))
